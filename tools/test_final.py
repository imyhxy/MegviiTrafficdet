...
def worker(
    ...
    for data in test_loader:
        # modify for ms test
        image_s, im_info_s = DetEvaluator.process_inputs(
            data[0][0],
            model.cfg.test_image_short_size,
            model.cfg.test_image_max_size,
        )
        box_cls_s, box_delta_s = [], []
        for i in range(len(im_info_s)):
            image = image_s[i]
            im_info = im_info_s[i]
            box_cls, box_delta = evaluator.predict(
                image=mge.tensor(image),
                im_info=mge.tensor(im_info)
            )
            box_cls_s.append(box_cls)
            box_delta_s.append(box_delta)
        box_cls_ = np.concatenate(box_cls_s, axis=0)
        box_delta_ = np.concatenate(box_delta_s, axis=0)
        dtboxes_all = list()
        test_cls_threshold = 0.05 ###
        all_inds = np.where(box_cls_ > test_cls_threshold)

        for c in range(5):
            inds = np.where(all_inds[1] == c)[0]
            inds = all_inds[0][inds]
            scores = box_cls_[inds, c]
            class_aware_box = False ###
            if class_aware_box:
                bboxes = box_delta_[inds, c, :]
            else:
                bboxes = box_delta_[inds, :]
            ...
            if dtboxes.size > 0:
                ...
                if test_nms == -1:
                    ...
                    soft=0 ###
                    if soft:
                        fin_bbox = py_soft_nms(dtboxes, test_nms)
                    else:
                        keep = py_cpu_nms(dtboxes, test_nms)
        ...
        if len(dtboxes_all) > test_max_boxes_per_image:
