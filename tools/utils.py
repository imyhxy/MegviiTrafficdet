...
from tools.nms import py_cpu_nms, py_soft_nms
ratios = [1.0]           # 0.562 647 449
ratios = [0.9, 1.0]      # 0.569 655 443
ratios = [0.8, 0.9, 1.0] # 0.574 659 440
ratios = [0.8, 0.9, 1.0, 1.1] # 0.576 663 449
ratios = [0.9, 1.0, 1.1] # 0.570 660 450
ratios = [0.7, 0.8, 0.9, 1.0, 1.1]# 0.574 664 444
ratios = [0.8, 0.85, 0.9, 0.95, 1.0, 1.05, 1.1]

#ratios = [0.8, 0.9, 1.0, 1.1]
ratios = [0.8, 0.9, 1.0, 1.1, 1.2] # 576 666 455   577 666 455
ratios = [0.75, 0.8, 0.9, 1.0, 1.1, 1.2]
ratios = [0.6666, 0.75, 0.8, 0.9, 1.0, 1.1, 1.2]
ratios = [0.75, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3]

ratios = [0.85, 0.95]    #
ratios = [0.8, 1.0]    #
#ratios = [0.9, 1.0]      # 0.569 655 443 # for final game 3.5h
ratios = [0.9]
print(ratios)
...
class DetEvaluator:
    ...
    #@staticmethod
    def process_inputs0(img, short_size, max_size, flip=False):
    ...
    def process_inputs(img, short_size, max_size, flip=False):
        original_height, original_width, _ = img.shape
        trans_img_s, im_info_s = [], []
        
        for ratio in ratios:
            short_size_, max_size_ = short_size*ratio, max_size*ratio
            resized_height, resized_width = DetEvaluator.get_hw_by_short_size(
                original_height, original_width, short_size_, max_size_
            )
            resized_img = cv2.resize(
                img, (resized_width, resized_height), interpolation=cv2.INTER_LINEAR,
            )
            ...
            trans_img_s.append(trans_img)
            im_info_s.append(im_info)
        #print(im_info_s)
        return trans_img_s, im_info_s
        ...
        for c in range(self.model.cfg.num_classes):
            ...
            if dtboxes.size > 0:
                if self.model.cfg.test_nms == -1:
                    ...
                    soft=1
                    if soft:
                        fin_bbox = py_soft_nms(dtboxes, self.model.cfg.test_nms)
                    else:
                        keep = py_cpu_nms(dtboxes, self.model.cfg.test_nms)
                        fin_bbox = dtboxes[keep]
                fin_dtboxes = np.hstack(
                    (fin_bbox, np.full((len(fin_bbox), 1), c, dtype=np.float32))
                ).astype(np.float32)

                dtboxes_all.extend(fin_dtboxes)
